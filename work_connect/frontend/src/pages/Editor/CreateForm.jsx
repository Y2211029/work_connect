...
const CreateForm = ({
  select_draft_list
    ...
    console.log("選択したリスト",select_draft_list);
    // Form の内容を useState にセットする例
    const [questions, setQuestions] = useState(getcreateform); // 配列を初期値として設定
    ...
    const [create_news_id] = useState(select_draft_list.news_id);
    ...
    console.log("クリエイトフォーム",select_draft_list.create_form.create_form);

    //createformがなかったらテンプレートのフォームデータを追加
    const getcreateform = () => {
      let createform = []; // 初期化
    
      // フォームデータが存在するかチェック
      if (select_draft_list.create_form[0].create_form) {
        // create_form が存在する場合、その内容をパースして設定
        createform = JSON.parse(select_draft_list.create_form[0].create_form);
      } else {
        // フォームデータが存在しない場合、テンプレートのフォームデータを設定
        createform = [
          {
            id: `1`,
            name: `Question1`,
    ...
    return (
      <>
        <Stack direction="row" spacing={2} style={{ width: "1000px" }}>
          ...
          <Typography>{select_draft_list.article_title}</Typography>
...
CreateForm.propTypes = {
  select_draft_list: PropTypes.shape({
    news_id: PropTypes.number,
    article_title: PropTypes.string,
    create_form: PropTypes.arrayOf(
      PropTypes.shape({
        id: PropTypes.number.isRequired,
        company_id: PropTypes.string,
        news_id: PropTypes.string,
        create_form: PropTypes.string.isRequired // JSON文字列としての create_form
      })
    ).isRequired
  })
};
